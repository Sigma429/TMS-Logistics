...
import com.sigma429.sl.enums.TruckPlanStatusEnum;
import com.sigma429.sl.exception.SLException;
import com.sigma429.sl.mapper.truck.TransportTripsTruckDriverMapper;
import com.sigma429.sl.service.truck.TransportTripsTruckDriverService;
import com.sigma429.sl.service.truck.TruckPlanCreateService;
 ...
 */
@Service
public class TransportTripsTruckDriverServiceImpl extends ServiceImpl<TransportTripsTruckDriverMapper,
        TransportTripsTruckDriverEntity>
    ...
    @Transactional
    @Override
    public void batchSave(Long truckTransportTripsId,
                          List<TransportTripsTruckDriverEntity> truckTransportTripsTruckDriverEntityList) {
        // 保存车辆和车次关联关系
        // 1,清除关系
        ...
        truckTransportTripsTruckDriverEntityList.forEach(transportTripsTruckDriver -> {
            List<TruckDriverEntity> driverEntities =
                    truckDriverService.findByTruckId(transportTripsTruckDriver.getTruckId());
            ...
            driverEntities.forEach(truckDriverEntity -> {
                TransportTripsTruckDriverEntity saveData = BeanUtil.toBean(transportTripsTruckDriver,
            ...
            // 触发创建首次车辆计划
            List<Long> driverIds =
                    driverEntities.stream().map(TruckDriverEntity::getUserId).collect(Collectors.toList());
    ...
    public void delete(Long transportTripsId, Long truckId) {
        // 删除车辆和车次关联关系
        LambdaQueryWrapper<TransportTripsTruckDriverEntity> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.eq(ObjectUtils.isNotEmpty(transportTripsId),
                        TransportTripsTruckDriverEntity::getTransportTripsId, transportTripsId)
