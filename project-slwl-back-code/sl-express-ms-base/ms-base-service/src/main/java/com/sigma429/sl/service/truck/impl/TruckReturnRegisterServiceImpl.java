...
import com.sigma429.sl.mapper.truck.TruckReturnRegisterMapper;
...
public class TruckReturnRegisterServiceImpl extends ServiceImpl<TruckReturnRegisterMapper, TruckReturnRegisterEntity> implements TruckReturnRegisterService {
    ...
    public PageResponse<TruckReturnRegisterListDTO> pageQuery(TruckReturnRegisterPageQueryDTO dto) {
        ...
        LambdaQueryWrapper<TruckReturnRegisterEntity> queryWrapper = Wrappers.<TruckReturnRegisterEntity>lambdaQuery()
                .like(ObjectUtil.isNotEmpty(dto.getTransportTaskId()), TruckReturnRegisterEntity::getTransportTaskId,
                        dto.getTransportTaskId())
                .in(ObjectUtil.isNotEmpty(dto.getTransportTaskIds()), TruckReturnRegisterEntity::getTransportTaskId,
                        dto.getTransportTaskIds())
                .between(ObjectUtil.isNotEmpty(dto.getIntoStorageStartTime()),
                        TruckReturnRegisterEntity::getIntoStorageTime, dto.getIntoStorageStartTime(),
                        dto.getIntoStorageEndTime())
                .eq(ObjectUtil.isNotEmpty(dto.getIsAvailable()), TruckReturnRegisterEntity::getIsAvailable,
                        dto.getIsAvailable())
    ...
    private List<TruckReturnRegisterListDTO> convertEntity2ListDTO(List<TruckReturnRegisterEntity> returnRegisterEntities) {
        // 先根据id批量查询车辆信息
        List<Long> truckIds =
                returnRegisterEntities.stream().map(TruckReturnRegisterEntity::getTruckId).collect(Collectors.toList());
        List<TruckEntity> truckEntities = truckService.listByIds(truckIds);
        return returnRegisterEntities.stream().map(entity -> {
            // 根据id查询车辆
            Optional<TruckEntity> optional =
                    truckEntities.stream().filter(item -> item.getId().equals(entity.getTruckId())).findFirst();
            if (optional.isEmpty()) {
                throw new SLException("id为：" + entity.getTruckId() + "车辆不存在！");
            }

            // 封装回车登记列表数据
            TruckReturnRegisterListDTO truckReturnRegisterListDTO = BeanUtil.toBean(entity,
                    TruckReturnRegisterListDTO.class);
