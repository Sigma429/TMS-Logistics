...
public class HistoryScheduleJob {
    ...
    public void execute() {
        ...
        // 查询当前排班
        List<WorkSchedulingEntity> workSchedulingEntities =
                workSchedulingMapper.selectList(Wrappers.<WorkSchedulingEntity>lambdaQuery().eq(WorkSchedulingEntity::getIsDelete, 0));

        // 查询工作模式
        List<Long> workPatternIds =
                workSchedulingEntities.parallelStream().map(WorkSchedulingEntity::getWorkPatternId).distinct().collect(Collectors.toList());
        ...
        // 补充日期数据
        List<WorkHistorySchedulingEntity> workHistorySchedulingEntities =
                workSchedulingEntities.parallelStream().map(workSchedulingEntity -> {
            WorkHistorySchedulingEntity workHistorySchedulingEntity = BeanUtil.toBean(workSchedulingEntity,
                    WorkHistorySchedulingEntity.class);
            // 重置ID
            ...
            // 月 前一天对应的月
            String workMonth = LocalDateTimeUtil.format(LocalDateTimeUtil.now().minusDays(1),
            ...
            // 只保存上班数据
            boolean worded = WorkSchedulingUtils.isWorded(workSchedulingEntity, workMonth, dayOfMonth,
                    workPatternEntity, null, true);
            if (worded) {
