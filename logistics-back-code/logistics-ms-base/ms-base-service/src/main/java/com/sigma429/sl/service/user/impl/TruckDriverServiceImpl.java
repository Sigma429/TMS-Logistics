...
import com.sigma429.sl.mapper.user.TruckDriverMapper;
...
public class TruckDriverServiceImpl extends ServiceImpl<TruckDriverMapper, TruckDriverEntity>
    ...
    private void checkCanBingingTruck(TruckDriverEntity oldDriver) {
        ...
        if (ObjectUtil.isNotEmpty(oldDriver.getTruckId())) {
            // 检查车次绑定关系
            List<TransportTripsTruckDriverEntity> all = transportTripsTruckDriverService.findAll(null,
                    oldDriver.getTruckId(), oldDriver.getUserId());
            if (all.size() > 0) {
                // 不能解除
                List<Long> tripsIds =
                        all.parallelStream().map(TransportTripsTruckDriverEntity::getTransportTripsId).distinct().collect(Collectors.toList());
                List<TruckTripsEntity> truckTripsEntities = truckTripsService.findAll(null, tripsIds);
                List<String> tripsNames =
                        truckTripsEntities.parallelStream().map(TruckTripsEntity::getName).distinct().collect(Collectors.toList());
                List<Long> lineIds =
                        truckTripsEntities.parallelStream().map(TruckTripsEntity::getTransportLineId).distinct().collect(Collectors.toList());
                List<TransportLineDTO> lineDTOS = transportLineFeign.queryByIds(lineIds.toArray(Long[]::new));
                List<String> names =
                        lineDTOS.parallelStream().map(TransportLineDTO::getName).collect(Collectors.toList());
