...
import com.sigma429.sl.enums.WorkStatusEnum;
...
public class WorkSchedulingServiceImpl extends ServiceImpl<WorkSchedulingMapper, WorkSchedulingEntity> implements WorkSchedulingService {
    ...
    public PageResponse<WorkSchedulingDTO> queryForPage(WorkSchedulingQueryDTO workSchedulingQueryDTO) {
        ...
        queryWrapper
                ...
                .eq(ObjectUtil.isNotEmpty(workSchedulingQueryDTO.getUserType()), WorkSchedulingEntity::getUserType,
                        workSchedulingQueryDTO.getUserType())
                .like(ObjectUtil.isNotEmpty(workSchedulingQueryDTO.getName()), WorkSchedulingEntity::getName,
                        workSchedulingQueryDTO.getName())
                .like(ObjectUtil.isNotEmpty(workSchedulingQueryDTO.getEmployeeNumber()),
                        WorkSchedulingEntity::getEmployeeNumber, workSchedulingQueryDTO.getEmployeeNumber())
                .like(ObjectUtil.isNotEmpty(workSchedulingQueryDTO.getAgencyId()), WorkSchedulingEntity::getAgencyId,
                        workSchedulingQueryDTO.getAgencyId())
                .eq(ObjectUtil.isNotEmpty(workSchedulingQueryDTO.getWorkPatternId()),
                        WorkSchedulingEntity::getWorkPatternId, workSchedulingQueryDTO.getWorkPatternId())
        ...
        return PageResponse.of(entityPage, WorkSchedulingDTO.class, (entity, dto) -> {
            ...
            for (int count = 1; count <= dayNumOfQueryMonth; count++) {
                workSchedules.add(WorkSchedulingUtils.isWorded(entity, queryMonth, count,
                        patternByIds.get(entity.getWorkPatternId()), historyMap, false));
            }
    ...
    public void add(WorkSchedulingAddDTO workSchedulingAddDTO) {
        WorkSchedulingEntity workSchedulingEntity = BeanUtil.toBean(workSchedulingAddDTO, WorkSchedulingEntity.class,
                (dto, entity) -> {
    ...
    public void batchAdd(List<WorkSchedulingAddDTO> workSchedulingAddDTOList) {
        ...
        for (int count = 0; count < workSchedulingAddDTOList.size(); count++) {
            WorkSchedulingEntity workSchedulingEntity = BeanUtil.toBean(workSchedulingAddDTOList.get(count),
                    WorkSchedulingEntity.class, (dto, entity) -> {
                        entity.setId(IdWorker.getId());
                        entity.setCreater(dto.getOperator());
                        entity.setUpdater(dto.getOperator());
                        entity.setCreated(LocalDateTime.now());
    ...
    public void update(WorkSchedulingUpdateDTO workSchedulingUpdateDTO) {
        ...
        WorkSchedulingEntity workSchedulingEntity = BeanUtil.toBean(workSchedulingUpdateDTO,
                WorkSchedulingEntity.class, (dto, entity) -> {
                    entity.setUpdated(LocalDateTime.now());
                    entity.setUpdater(dto.getOperator());
     ...
     * @return 工作模式
     */
    private WorkSchedulingDTO appenWorkPatten(WorkSchedulingDTO workSchedulingDTO,
                                              Map<Long, WorkPatternEntity> workPatternEntityMap) {
        ...
        return BeanUtil.toBean(entity, WorkSchedulingDTO.class, (workSchedulingEntity, workSchedulingDTO) -> {
            ...
            if (workSchedulingEntity.getWorkPatternId() == 0) {
                ...
                for (int count = currentDay; count <= monthNumber; count++) {
                    boolean worded = WorkSchedulingUtils.isWorded(entity, LocalDateTimeUtil.format(startTime,
                                    DateUtils.DEFAULT_MONTH_FORMAT),
    ...
    private Map<Long, WorkPatternEntity> getPatternByIds(List<WorkSchedulingEntity> records) {
        ...
        // 补充工作模式名称
        List<Long> workPatternIds =
                records.stream().map(WorkSchedulingEntity::getWorkPatternId).collect(Collectors.toList());
     ...
     */
    @Override
    public List<Long> getWorkingDrivers(List<Long> driverIds, LocalDateTime planDepartureTime,
                                        LocalDateTime planArrivalTime) {
        ...
        List<Long> workUserIds = list.stream().filter(workSchedulingEntity -> {
            ...
            for (LocalDateTime count = planDepartureTime; count.isBefore(planArrivalTime); count = count.plusDays(1)) {
                ...
                // 计算这一天的排班情况
                boolean worded = WorkSchedulingUtils.isWorded(workSchedulingEntity, month, dayOfMonth,
                        patternByIds.get(workSchedulingEntity.getWorkPatternId()), null, false);
