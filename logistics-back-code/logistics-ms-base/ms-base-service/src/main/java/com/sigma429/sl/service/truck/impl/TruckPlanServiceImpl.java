...
import com.sigma429.sl.DispatchConfigurationFeign;
...
public class TruckPlanServiceImpl extends ServiceImpl<TruckPlanMapper, TruckPlanEntity>
    ...
    private List<TruckPlanDto> toUnassignedDto(List<TruckPlanEntity> records, Integer shardTotal, Integer shardIndex) {
        ...
        List<Long> transportTripsIds =
                truckPlanDtos.parallelStream().map(TruckPlanDto::getTransportTripsId).collect(Collectors.toList());
        ...
        // 根据线路ID查询起始位置
        Map<Long, OrganIdsDto> organIdsMap =
                truckTripsService.getOrganIdsByTripsLineId(new HashSet<>(truckTripsLineMap.values()));
    ...
    public void finishedPlan(Long currentOrganId, Long planId, Long truckId, StatusEnum statusEnum) {
        ...
        CompletableFuture<String> nextPlans = truckPlanCreateService.createNextPlans(truckPlanEntity.getTruckId(),
                driverIds, currentOrganId);
