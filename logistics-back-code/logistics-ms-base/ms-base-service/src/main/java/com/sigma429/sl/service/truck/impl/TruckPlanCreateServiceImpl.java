...
import com.sigma429.sl.enums.TruckTripsPeriodEnum;
import com.sigma429.sl.exception.SLException;
import com.sigma429.sl.mapper.truck.TruckPlanMapper;
...
public class TruckPlanCreateServiceImpl extends ServiceImpl<TruckPlanMapper, TruckPlanEntity>
    ...
    public void createPlan(TruckPlanEntity truckPlanEntity) {
        ...
        LocalDateTime planDepartureTime =
                DateUtils.getStartTime(LocalDateTime.now()).plusMinutes(truckTripsEntity.getDepartureTime());
        truckPlanEntity.setPlanDepartureTime(planDepartureTime);
        // 计划到达时间
        LocalDateTime planArrivalTime =
                truckPlanEntity.getPlanDepartureTime().plusMinutes(truckTripsEntity.getEstimatedTime());
        ...
        List<Long> driverIds =
                truckDriverEntities.stream().map(TruckDriverEntity::getUserId).collect(Collectors.toList());
        List<Long> workingDrivers = workSchedulingService.getWorkingDrivers(driverIds, planDepartureTime,
                planArrivalTime);
    ...
    private void createNextPlan(Long truckId, List<Long> driverIds, Long transportTripsId) {
        ...
        // 按照最新的车次时间设置计划发车时间
        LocalDateTime planDepartureTimeNew =
                DateUtils.getStartTime(planDepartureTime).plusMinutes(truckTripsEntity.getDepartureTime());
