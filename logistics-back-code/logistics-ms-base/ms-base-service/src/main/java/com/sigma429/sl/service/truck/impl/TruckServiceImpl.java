...
import com.sigma429.sl.mapper.truck.TruckMapper;
...
public class TruckServiceImpl extends ServiceImpl<TruckMapper, TruckEntity> implements TruckService {
     ...
     */
    @Override
    public IPage<TruckEntity> findByPage(Integer page, Integer pageSize, Long truckTypeId, Integer status,
                                         String licensePlate) {
    ...
    public void workedById(Long id) {
        ...
        List<Long> drivers =
                driverEntities.parallelStream().map(TruckDriverEntity::getUserId).collect(Collectors.toList());
        // 有排班才可以启用
