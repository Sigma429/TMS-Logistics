...
import com.sigma429.sl.service.OrderService;
...
public class OrderController {
    ...
    public PageResponse<OrderDTO> findByPage(@RequestBody OrderDTO orderDTO) {
        IPage<OrderEntity> orderIPage = crudOrderService.findByPage(orderDTO.getPage(), orderDTO.getPageSize(),
                orderDTO);
    ...
    public List<OrderDTO> list(@RequestBody OrderSearchDTO orderSearchDTO) {
        ...
        wrapper.eq(ObjectUtil.isNotEmpty(orderSearchDTO.getStatus()), OrderEntity::getStatus,
                orderSearchDTO.getStatus());
        wrapper.in(CollUtil.isNotEmpty(orderSearchDTO.getReceiverCountyIds()), OrderEntity::getReceiverCountyId,
                orderSearchDTO.getReceiverCountyIds());
        wrapper.in(CollUtil.isNotEmpty(orderSearchDTO.getSenderCountyIds()), OrderEntity::getSenderCountyId,
                orderSearchDTO.getSenderCountyIds());
        wrapper.eq(ObjectUtil.isNotEmpty(orderSearchDTO.getCurrentAgencyId()), OrderEntity::getCurrentAgencyId,
                orderSearchDTO.getCurrentAgencyId());

        // 快递员端进行条件查询取派件任务时，会传入关键词，作为订单id或姓名或手机号查询条件
        if (StringUtils.isNotBlank(orderSearchDTO.getKeyword())) {
            wrapper.like(ObjectUtil.isNotEmpty(orderSearchDTO.getOrderId()), OrderEntity::getId,
                            orderSearchDTO.getKeyword())
                    .or().like(ObjectUtil.isNotEmpty(orderSearchDTO.getSenderName()), OrderEntity::getSenderName,
                            orderSearchDTO.getSenderName())
                    .or().like(ObjectUtil.isNotEmpty(orderSearchDTO.getSenderPhone()), OrderEntity::getSenderPhone,
                            orderSearchDTO.getSenderPhone())
                    .or().like(ObjectUtil.isNotEmpty(orderSearchDTO.getReceiverName()), OrderEntity::getReceiverName,
                            orderSearchDTO.getReceiverName())
                    .or().like(ObjectUtil.isNotEmpty(orderSearchDTO.getReceiverPhone()),
                            OrderEntity::getReceiverPhone, orderSearchDTO.getReceiverPhone());
        }
