...
import com.sigma429.sl.enums.MailType;
import com.sigma429.sl.enums.OrderPaymentStatus;
import com.sigma429.sl.enums.OrderPickupType;
import com.sigma429.sl.enums.OrderStatus;
import com.sigma429.sl.exception.SLException;
import com.sigma429.sl.mapper.OrderMapper;
import com.sigma429.sl.service.CrudOrderService;
import com.sigma429.sl.service.OrderCargoService;
...
public class CrudOrderServiceImpl extends ServiceImpl<OrderMapper, OrderEntity> implements CrudOrderService {
    ...
    public Page<OrderEntity> findByPage(Integer page, Integer pageSize, OrderDTO orderDTO) {
        ...
        lambdaQueryWrapper
                ...
                .eq(ObjectUtil.isNotEmpty(order.getPaymentStatus()), OrderEntity::getPaymentStatus,
                ...
                .eq(ObjectUtil.isNotEmpty(order.getSenderProvinceId()), OrderEntity::getSenderProvinceId,
                        order.getSenderProvinceId())
                .eq(ObjectUtil.isNotEmpty(order.getSenderCityId()), OrderEntity::getSenderCityId,
                        order.getSenderCityId())
                .eq(ObjectUtil.isNotEmpty(order.getSenderCountyId()), OrderEntity::getSenderCountyId,
                        order.getSenderCountyId())

                // 收件人信息
                .like(ObjectUtil.isNotEmpty(order.getReceiverName()), OrderEntity::getReceiverName,
                        order.getReceiverName())
                .eq(ObjectUtil.isNotEmpty(order.getReceiverProvinceId()), OrderEntity::getReceiverProvinceId,
                        order.getReceiverProvinceId())
                .eq(ObjectUtil.isNotEmpty(order.getReceiverCityId()), OrderEntity::getReceiverCityId,
                        order.getReceiverCityId())
                .eq(ObjectUtil.isNotEmpty(order.getReceiverCountyId()), OrderEntity::getReceiverCountyId,
                        order.getReceiverCountyId())
        ...
        // 收件人手机号查询 客户端、管理端共用
        lambdaQueryWrapper
                .eq(isQueryByCustom && StrUtil.isNotEmpty(order.getReceiverPhone()), OrderEntity::getReceiverPhone,
                        order.getReceiverPhone())
                .like(!isQueryByCustom && StrUtil.isNotEmpty(order.getReceiverPhone()), OrderEntity::getReceiverPhone
                        , order.getReceiverPhone())
                // 客户端非寄件列表 不展示这些状态
                .notIn(isQueryByCustom && !MailType.SEND.getCode().equals(orderDTO.getMailType()),
                        OrderEntity::getStatus, Arrays.asList(OrderStatus.CLOSE.getCode(),
                                OrderStatus.CANCELLED.getCode(), OrderStatus.PENDING.getCode()));

        // 客户端 寄件列表
        lambdaQueryWrapper.eq(isQueryByCustom && MailType.SEND.getCode().equals(orderDTO.getMailType()),
                OrderEntity::getMemberId, order.getMemberId());
        ...
        lambdaQueryWrapper.or(isQueryByCustom && MailType.ALL.getCode().equals(orderDTO.getMailType()))
                .eq(isQueryByCustom && MailType.ALL.getCode().equals(orderDTO.getMailType()),
                        OrderEntity::getMemberId, order.getMemberId())
                .ne(isQueryByCustom && MailType.ALL.getCode().equals(orderDTO.getMailType()), OrderEntity::getStatus,
    ...
    public List<OrderStatusCountDTO> groupByStatus(Long memberId) {
        ...
        long count = count(Wrappers.<OrderEntity>lambdaQuery()
                ...
                .ne(OrderEntity::getStatus, OrderStatus.DEL.getCode())
                .notIn(OrderEntity::getStatus, Arrays.asList(OrderStatus.CLOSE.getCode(),
                        OrderStatus.CANCELLED.getCode(), OrderStatus.PENDING.getCode()))
        );
