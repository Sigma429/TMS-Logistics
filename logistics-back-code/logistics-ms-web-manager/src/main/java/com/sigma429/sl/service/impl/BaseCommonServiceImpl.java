...
import com.sigma429.sl.base.*;
import com.sigma429.sl.common.AreaFeign;
import com.sigma429.sl.common.GoodsTypeFeign;
import com.sigma429.sl.common.WorkPatternFeign;
import com.sigma429.sl.common.WorkSchedulingFeign;
import com.sigma429.sl.enums.TruckTypeAllowableLoadEnum;
import com.sigma429.sl.enums.TruckTypeAllowableVolumeEnum;
import com.sigma429.sl.exception.SLWebException;
import com.sigma429.sl.service.AuthService;
...
import com.sigma429.sl.vo.auth.SysUserVO;
import com.sigma429.sl.vo.baseCommon.*;
import com.sigma429.sl.vo.baseTruck.TruckTypeVO;
...
public class BaseCommonServiceImpl implements BaseCommonService {
    ...
    public void batchSaveWorkScheduling(WorkSchedulingAddVO workSchedulingAddVO) {
        Map<Long, SysUserVO> sysUserVOMap =
                authService.users(workSchedulingAddVO.getUserIdList()).parallelStream().collect(Collectors.toMap(SysUserVO::getUserId, v -> v));
    ...
    public PageResponse<WorkSchedulingDTO> listWorkScheduling(WorkSchedulingQueryDTO workSchedulingQueryDTO) {
        ...
        // 以权限系统为主
        List<Long> userIds =
                pageResponse.getItems().parallelStream().map(WorkSchedulingDTO::getUserId).collect(Collectors.toList());
        Map<Long, SysUserVO> sysUserVOMap =
                authService.users(userIds).parallelStream().collect(Collectors.toMap(SysUserVO::getUserId, v -> v));
    ...
    public void batchSaveWorkScheduling(List<WorkSchedulingExportVO> list) {
        List<WorkSchedulingAddDTO> collect =
                list.stream().map(v -> BeanUtil.toBean(v, WorkSchedulingAddDTO.class)).collect(Collectors.toList());
     ...
     */
    @Override
    public PageResponse<GoodsTypeVO> findGoodsTypeByPage(Integer page, Integer pageSize, String name,
                                                         Long truckTypeId) {
    ...
    public GoodsTypeVO parseGoodsTypeDto2Vo(GoodsTypeDto dto) {
        ...
        if (ObjectUtil.isNotEmpty(dto) && ObjectUtil.isNotEmpty(dto.getTruckTypeIds())) {
            ...
            if (ObjectUtil.isNotEmpty(truckTypeDtoList)) {
                vo.setTruckTypes(truckTypeDtoList.stream().map(truckTypeDto -> BeanUtil.toBean(truckTypeDto,
                        TruckTypeVO.class)).collect(Collectors.toList()));
            }
        }
        return vo;
    }
    ...
    public PageResponse<TruckTypeVO> findTruckTypeByPage(Integer page,
                                                         ...
                                                         Long id) {
        // 构造查询条件
        BigDecimal minAllowableLoad = ObjectUtil.isNotEmpty(allowableLoad) ? allowableLoad.getValue().getMinLoad() :
                null;
        BigDecimal maxAllowableLoad = ObjectUtil.isNotEmpty(allowableLoad) ? allowableLoad.getValue().getMaxLoad() :
                null;
        BigDecimal minAllowableVolume = ObjectUtil.isNotEmpty(allowableVolume) ?
                allowableVolume.getValue().getMinLoad() : null;
        BigDecimal maxAllowableVolume = ObjectUtil.isNotEmpty(allowableVolume) ?
                allowableVolume.getValue().getMaxLoad() : null;

        // 分页查询
        PageResponse<TruckTypeDto> truckTypeDtoPage = truckTypeFeign.findByPage(page, pageSize, name,
                minAllowableLoad, maxAllowableLoad, minAllowableVolume, maxAllowableVolume, id);
