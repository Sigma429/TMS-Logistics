...
public class OmsServiceImpl implements OmsService {
    ...
    public Map<Integer, Long> count() {
        Long userId = UserThreadLocal.getUserId();
        return orderFeign.count(userId).stream().collect(Collectors.toMap(OrderStatusCountDTO::getStatusCode,
    ...
    public void cancel(Long id) {
        ...
        // 查询取派件任务
        List<PickupDispatchTaskDTO> pickupDispatchTaskDTOS = pickupDispatchTaskFeign.findByOrderId(id,
    ...
    public List<OrderVO> batchParseOrderDTO2Vo(List<OrderDTO> orderDTOList) {
        ...
        Map<Long, String> transportOrderIdMap =
                transportOrderFeign.findByOrderIds(orderDTOList.parallelStream().map(OrderDTO::getId).collect(Collectors.toList()).toArray(Long[]::new)).parallelStream().collect(Collectors.toMap(TransportOrderDTO::getOrderId, TransportOrderDTO::getId));
