...
import com.sigma429.sl.dto.response.DailyMileageDTO;
...
import com.sigma429.sl.entity.DriverJobEntity;
...
import com.sigma429.sl.mapper.DriverJobMapper;
import com.sigma429.sl.service.DriverJobService;
...
public class DriverJobServiceImpl extends ServiceImpl<DriverJobMapper, DriverJobEntity> implements DriverJobService {
    ...
    public Long createDriverJob(Long transportTaskId, Long driverId) {
        ...
        // 计划发车时间
        driverJobEntity.setPlanDepartureTime(truckPlanDto.getPlanDepartureTime());
        // 实际到达时间
        driverJobEntity.setPlanArrivalTime(truckPlanDto.getPlanArrivalTime());
        ...
        if (result) {
            ...
            String content = CharSequenceUtil.format("运输车辆：{}\n运输路线：{}——{}", truckDto.getLicensePlate(),
                    startOrgan.getName(), endOrgan.getName());
    ...
    public void intoStorage(DriverDeliverDTO driverDeliverDTO) {
        ...
        if (lock.tryLock()) {
            ...
            try {
                ...
                if (ObjectUtil.equalsAny(transportTask.getStatus(), TransportTaskStatus.CANCELLED,
                ...
                // 2.6结束运输任务
                TransportTaskCompleteDTO transportTaskCompleteDTO = BeanUtil.toBean(driverDeliverDTO,
                        TransportTaskCompleteDTO.class);
    ...
    public void outStorage(DriverPickUpDTO driverPickUpDTO) {
        ...
        if (lock.tryLock()) {
            ...
            try {
                ...
                if (ObjectUtil.equalsAny(transportTask.getStatus(), TransportTaskStatus.PROCESSING,
                        TransportTaskStatus.CONFIRM)) {
                    return;
                }

                // 2.5修改运单状态为运输中
                List<String> transportOrderIdList =
                        this.transportTaskFeign.queryTransportOrderIdListById(transportTaskId);
                this.transportOrderFeign.updateStatus(transportOrderIdList, TransportOrderStatus.PROCESSING);

                // 2.6开始运输任务
                TransportTaskStartDTO transportTaskStartDTO = BeanUtil.toBean(driverPickUpDTO,
                        TransportTaskStartDTO.class);
    ...
    public void delayedDelivery(DriverDelayDeliveryDTO driverDelayDeliveryDTO) {
        ...
        LocalDateTime delayTime = LocalDateTimeUtil.parse(driverDelayDeliveryDTO.getDelayTime(),
        ...
        // 3.更改运输任务计划发车时间和备注
        TransportTaskDelayDeliveryDTO transportTaskDelayDeliveryDTO = BeanUtil.toBean(driverDelayDeliveryDTO,
                TransportTaskDelayDeliveryDTO.class);
    ...
    public void returnRegister(DriverReturnRegisterDTO driverReturnRegisterDTO) {
        ...
        // 构建回车登记对象
        TruckReturnRegisterDTO truckReturnRegisterDTO = BeanUtil.toBean(driverReturnRegisterDTO,
                TruckReturnRegisterDTO.class);
    ...
    public DriverJobStatisticsDTO jobMonthlyStatistics(String driverId, String month) {
        ...
        // 从司机作业单中获取运输任务id列表
        List<String> transportTaskIds =
                completedList.stream().map(x -> String.valueOf(x.getTransportTaskId())).collect(Collectors.toList());

        // 根据实际到达时间查询运输任务每日距离
        List<DailyMileageDTO> dailyMileageDTOS = new ArrayList<>();
        long transportMileage = 0L;
        if (ObjectUtil.isNotEmpty(transportTaskIds)) {
            List<TransportTaskMonthlyDistanceDTO> transportTaskMonthlyDistanceDTOS =
                    transportTaskFeign.monthlyDistanceStatistics(transportTaskIds, month);
    ...
    public PageResponse<DriverJobDTO> pageQuery(DriverJobPageQueryDTO dto) {
        ...
        LambdaQueryWrapper<DriverJobEntity> queryWrapper = Wrappers.<DriverJobEntity>lambdaQuery()
                ...
                .eq(ObjectUtil.isNotEmpty(dto.getTransportTaskId()), DriverJobEntity::getTransportTaskId,
                        dto.getTransportTaskId())
        ...
        if (CollUtil.isNotEmpty(dto.getStatusList()) && dto.getStatusList().contains(DriverJobStatus.PENDING)) {
            queryWrapper
                    .between(ObjectUtil.isNotEmpty(dto.getMinTaskTime()), DriverJobEntity::getPlanDepartureTime,
                            dto.getMinTaskTime(), dto.getMaxTaskTime())
            ...
            queryWrapper
                    .between(ObjectUtil.isNotEmpty(dto.getMinTaskTime()), DriverJobEntity::getActualArrivalTime,
                            dto.getMinTaskTime(), dto.getMaxTaskTime())
            ...
            queryWrapper
                    .between(ObjectUtil.isNotEmpty(dto.getMinTaskTime()), DriverJobEntity::getPlanArrivalTime,
                            dto.getMinTaskTime(), dto.getMaxTaskTime())
                    .orderByAsc(DriverJobEntity::getPlanArrivalTime);
        } else {
            queryWrapper
                    .between(ObjectUtil.isNotEmpty(dto.getMinTaskTime()), DriverJobEntity::getCreated,
                            dto.getMinTaskTime(), dto.getMaxTaskTime())
