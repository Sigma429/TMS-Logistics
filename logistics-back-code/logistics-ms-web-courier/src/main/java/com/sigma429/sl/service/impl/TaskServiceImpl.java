...
import com.sigma429.sl.constants.CourierConstants;
import com.sigma429.sl.constants.MessageConstants;
import com.sigma429.sl.dto.*;
...
import com.sigma429.sl.enums.pickupDispatchtask.SignRecipientEnum;
...
public class TaskServiceImpl implements TaskService {
    ...
    private TaskVO getTaskVO(PickupDispatchTaskDTO taskDTO, Map<Long, OrderDTO> orderMap) {
        ...
        taskVO.setCancelReason(ObjectUtil.isEmpty(taskDTO.getCancelReason()) ? null :
                taskDTO.getCancelReason().getValue());
    ...
    private void assembleTaskVO(TaskVO taskVO, Map<Long, OrderDTO> orderMap) {
        ...
        if (PickupDispatchTaskType.DISPATCH.equals(taskVO.getTaskType())) {
            List<PickupDispatchTaskDTO> pickupDispatchTaskDTOS =
                    pickupDispatchTaskFeign.findByOrderId(taskVO.getOrderId(), taskVO.getTaskType());
    ...
    private Map<String, String> getNameAndPhoneAndAddress(TaskInformationEnum taskInformationEnum, OrderDTO orderDTO) {
        ...
        switch (taskInformationEnum) {
            case SEND: // 任务关联寄件人信息
                ...
                address = getDetailAddress(orderDTO.getSenderProvinceId(), orderDTO.getSenderCityId(),
            ...
            case RECEIVE:// 2任务关联收件人信息
                ...
                // 根据id查询省、市、区名称并拼接为完整地址
                address = getDetailAddress(orderDTO.getReceiverProvinceId(), orderDTO.getReceiverCityId(),
    ...
    private String getDetailAddress(Long provinceId, Long cityId, Long countyId, String address) {
        ...
        Map<Long, String> areaMap = areaList.stream().collect(Collectors.toMap(AreaDto::getId, AreaDto::getName, (key
                , value) -> value));
        return CharSequenceUtil.strBuilder(areaMap.get(provinceId), areaMap.get(cityId), areaMap.get(countyId),
                address).toString();
    }
    ...
    public PageResponse<TaskVO> pageQuery(TaskQueryVO vo) {
        ...
        // 3.按日期过滤任务
        taskList =
                taskList.stream().filter(task -> task.getTaskTime().isAfter(vo.getDateTime()) && task.getTaskTime().isBefore(vo.getDateTime().plusDays(1))).collect(Collectors.toList());
        ...
        if (ObjectUtil.isEmpty(vo.getStatus())) {
            taskList = taskList.stream().filter(task -> ObjectUtil.notEqual(task.getStatus(),
                    PickupDispatchTaskStatus.CANCELLED)).collect(Collectors.toList());
        }
        ...
        List<String> orderIds =
                taskList.stream().map(task -> task.getOrderId().toString()).collect(Collectors.toList());
    ...
    public TaskInfoVO detail(String id) {
        ...
        // 根据id查询省、市、区名称并拼接为完整地址
        String senderAddress = getDetailAddress(orderDTO.getSenderProvinceId(), orderDTO.getSenderCityId(),
                orderDTO.getSenderCountyId(), orderDTO.getSenderAddress());
    ...
    public boolean pickup(TaskPickupVO vo) {
        ...
        // 7.发送取件成功的消息
        String msg =
                CourierMsg.builder().orderId(orderId).courierId(userDTO.getId()).courierName(userDTO.getName()).courierMobile(userDTO.getMobile()).created(System.currentTimeMillis())
    ...
    public void cancel(TasksCancelVO vo) {
        ...
        Map<String, String> stringMap =
                this.getNameAndPhoneAndAddress(TaskInformationEnum.codeOf(taskDTO.getTaskType().getCode()), orderDTO);
        String content = CharSequenceUtil.format("地址：{}，寄件人：{}，电话：{}",
                stringMap.get(CourierConstants.OrderInfo.ADDRESS), stringMap.get(CourierConstants.OrderInfo.NAME),
                stringMap.get(CourierConstants.OrderInfo.ADDRESS));
    ...
    public void sign(TaskSignVO vo) {
        ...
        TaskInformationEnum flag = TaskInformationEnum.SEND;
        List<PickupDispatchTaskDTO> pickupDispatchTaskDTOS =
                pickupDispatchTaskFeign.findByOrderId(taskDTO.getOrderId(), PickupDispatchTaskType.DISPATCH);
        // pickupDispatchTaskDTOS是按创建时间正序排序的，第一条数据是正常的派件任务
        ...
        String content = CharSequenceUtil.format("地址：{}，收件人：{}，电话：{}",
                stringMap.get(CourierConstants.OrderInfo.ADDRESS), stringMap.get(CourierConstants.OrderInfo.NAME),
                stringMap.get(CourierConstants.OrderInfo.PHONE));
        ...
        // 4.1获取快递员信息
        UserDTO userDTO = AuthTemplateThreadLocal.get().opsForUser().getUserById(UserThreadLocal.getUserId()).getData();
        String info = CharSequenceUtil.format("您的快递已签收，如有疑问请联系快递员【{}，电话{}】，感谢您使用神领快递，期待再次为您服务", userDTO.getName(),
                userDTO.getMobile());
        ...
        // 4.3构建消息实体类
        TransportInfoMsg transportInfoMsg = TransportInfoMsg.builder().transportOrderId(transportOrderDTO.getId())//
                // 运单id
                .status("已签收")// 消息状态
                .info(info)// 消息详情
        ...
        // 4.4发送消息
        this.mqFeign.sendMsg(Constants.MQ.Exchanges.TRANSPORT_INFO, Constants.MQ.RoutingKeys.TRANSPORT_INFO_APPEND,
                transportInfoMsg.toJson());
        ...
        this.mqFeign.sendMsg(Constants.MQ.Exchanges.TRANSPORT_ORDER_DELAYED,
                Constants.MQ.RoutingKeys.TRANSPORT_ORDER_UPDATE_STATUS_PREFIX + "RECEIVED",
                transportOrderStatusMsg.toJson());
    }
    ...
    public void reject(String id) {
        ...
        // 5.1获取快递员信息
        UserDTO userDTO = AuthTemplateThreadLocal.get().opsForUser().getUserById(UserThreadLocal.getUserId()).getData();
        String info = CharSequenceUtil.format("您的快递已拒收，快递将返回到网点，如有疑问请电联快递员【联系人{}，电话：{}】", userDTO.getName(),
                userDTO.getMobile());

        // 5.2构建消息实体类
        TransportInfoMsg transportInfoMsg =
                TransportInfoMsg.builder().transportOrderId(transportOrderDTO.getId()).status("已拒收").info(info).created(DateUtil.current()).build();
    ...
    public List<String> recentSearch() {
        ...
        // 3.json转为对象
        List<RecentSearchVO> list =
                jsonList.stream().map(json -> JSONUtil.toBean(json, RecentSearchVO.class)).collect(Collectors.toList());
    ...
    public void markRecent(String transportOrderId) {
        ...
        RecentSearchVO recentSearchVO =
                RecentSearchVO.builder().transportOrderId(transportOrderId).created(LocalDateTime.now()).build();
        ...
        if (jsonList == null) {
            ...
            List<RecentSearchVO> recentSearchList = jsonList.stream().map(json -> JSONUtil.toBean(json,
                    RecentSearchVO.class)).collect(Collectors.toList());
            List<String> transportOrderIds =
                    recentSearchList.stream().map(RecentSearchVO::getTransportOrderId).collect(Collectors.toList());
    ...
    public PageResponse<TaskVO> search(TaskSearchVO taskSearchVO) {
        ...
        // 3.根据取派件id列表查询取派件任务
        List<Long> courierTaskIds =
                pageResponse.getItems().stream().map(CourierTaskDTO::getId).collect(Collectors.toList());
        List<PickupDispatchTaskDTO> pickupDispatchTaskDTOS = pickupDispatchTaskFeign.findByIds(courierTaskIds);

        // 4.将订单、运单相关数据封装
        List<String> orderIds =
                pickupDispatchTaskDTOS.stream().map(task -> task.getOrderId().toString()).collect(Collectors.toList());
        List<OrderDTO> orderDTOS = orderFeign.findByIds(orderIds);
        Map<Long, OrderDTO> orderMap = orderDTOS.stream().collect(Collectors.toMap(OrderDTO::getId, order -> order));
        List<TaskVO> taskVOList =
                pickupDispatchTaskDTOS.stream().map(task -> getTaskVO(task, orderMap)).collect(Collectors.toList());
        ...
        // 8.分页结果封装
        return PageResponse.of(taskVOList, pageResponse.getPage(), pageResponse.getPageSize(),
                pageResponse.getPages(), pageResponse.getCounts());
    }
}
