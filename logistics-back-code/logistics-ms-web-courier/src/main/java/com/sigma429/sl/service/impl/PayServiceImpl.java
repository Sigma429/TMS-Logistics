...
public class PayServiceImpl implements PayService {
    ...
    public TradeResponseVO getQrCode(TradeLaunchVO tradeLaunchVO) {
        ...
        Long enterpriseId = ObjectUtil.equal(payChannelEnum, PayChannelEnum.ALI_PAY) ? aliEnterpriseId :
                wechatEnterpriseId;

        // 3.如果已有，则对比数据库中和入参中的支付渠道是否相同
        // 3.1.如果相同，则使用已有交易号获取支付二维码
        TradeResponseVO tradeResponseVO = new TradeResponseVO();
        if (ObjectUtil.isNotEmpty(orderDTO.getTradingOrderNo()) && ObjectUtil.equal(payChannel,
                orderDTO.getTradingChannel())) {
